...
public class CSVUtil {
	...
	public static List<ParticipantsDTO> csvToParticipant(InputStream uploadedInputStream) throws ParseException, IOException {
		...
		while ((line = bfReader.readLine()) != null) {
            ...
            p.setLast_name_3(EncryptService.encryptStringDB(lastName.substring(0, Math.max(Constants.MAX_SUBSTRING_LENGHT_ENCRYPTED, lastName.length())).toLowerCase()));
            p.setFirst_name_3(EncryptService.encryptStringDB(name.toLowerCase().substring(0, Math.max(Constants.MAX_SUBSTRING_LENGHT_ENCRYPTED, name.length())).toLowerCase()));
            p.setNo_pcp(false);
            participants.add(p);
        }
		return participants;
