...
public class ClientAssessmentConstants {
	...
	public static final String SELECT_CLIENT_ASSESMENTS = "SELECT cca.*,cc.name as client_name,count(*) OVER() AS full_count FROM comed_client_assessment as cca inner join comed_clients cc on cca.client_id = cc.id WHERE cca.status = true OFFSET ? LIMIT ?;";
